 ...
 *
 * @method     ChildReferenceQuery leftJoinSkillReference($relationAlias = null) Adds a LEFT JOIN clause to the query using the SkillReference relation
 * @method     ChildReferenceQuery rightJoinSkillReference($relationAlias = null) Adds a RIGHT JOIN clause to the query using the SkillReference relation
 * @method     ChildReferenceQuery innerJoinSkillReference($relationAlias = null) Adds a INNER JOIN clause to the query using the SkillReference relation
 *
 * @method     \gossi\trixionary\model\VideoQuery|\gossi\trixionary\model\SkillReferenceQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class ReferenceQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `type`, `title`, `year`, `publisher`, `journal`, `number`, `school`, `author`, `edition`, `volume`, `address`, `editor`, `howpublished`, `note`, `booktitle`, `pages`, `url`, `lastchecked`, `managed` FROM `kk_trixionary_reference` WHERE `id` = :p0';
        try {
    ...
    public function filterBySkillReference($skillReference, $comparison = null)
    {
        if ($skillReference instanceof \gossi\trixionary\model\SkillReference) {
            return $this
                ->addUsingAlias(ReferenceTableMap::COL_ID, $skillReference->getReferenceId(), $comparison);
